# Hexa — Clean Core Assessment for GitHub Copilot

Adds a `/hexa-assess` slash command to GitHub Copilot Chat that assesses SAP
**clean-core ATC findings** and routes each into a remediation-effort bucket — AI Fix /
AI + Developer Fix / Architectural Redesign / Assessment Inconclusive — then generates a
**Clean Core Assessment** HTML report. It **assesses and categorizes only** — it does
not produce remediated ABAP.

## In scope — ATC findings

Each finding type has its own assessment module. **Active** = assessed today;
**In progress** = recognized but not yet assessed (reported as out of scope).

| ATC check | Finding message | Status |
|-----------|-----------------|--------|
| Usage of APIs | Updating DDIC database tables or DDIC table views is not allowed *(± successor available)* | ✅ Active |
| Usage of APIs | PERFORM program is not allowed | ✅ Active |
| Detect customer modifications | Object is modified | ✅ Active |
| Allowed Enhancement Technologies | Enhancement technology not allowed | 🚧 In progress |
...
- **Scope**: the ATC findings in **In scope** above — active modules are assessed;
  in-progress and unlisted finding types are marked out of scope.
